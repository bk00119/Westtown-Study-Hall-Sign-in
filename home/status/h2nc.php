...
if ($_SESSION["location"] != "DA")  {
  ...
  AND signins.date = CURRENT_DATE AND signins.sign_ts IS NOT NULL AND signins.confirm_ts_2 IS NULL";
  ...
  $result = mysqli_query($mysqli, $query);
      ...
      <?php
        while($row = $result->fetch_assoc())
        {
          ...
          <tr>
            ...
            <td>
              <button type="button" onclick="confirm_SI(<?php echo $row["record_id"];?>, 2)">Confirm</button>
            </td>
          </tr>
            <?php
        }
    ...
    AND signins.sign_ts IS NOT NULL AND signins.confirm_ts_2 IS NULL";
    $result = mysqli_query($mysqli, $query);
    if ($result->num_rows > 0){
      ?>
    <tr>
